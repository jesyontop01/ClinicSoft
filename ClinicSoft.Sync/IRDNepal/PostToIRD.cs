...
namespace ClinicSoft.Sync.IRDNepal
{
    ...
    {
        public static string bilConString =  string.Empty;
        static string maxTxnRecords =  string.Empty;
        ...
        public static void SyncSalesToRemoteServer()
        {
            ...
            foreach (BillingTransactionModel billTxn in unSyncedBillTxns)
            {
                ...
                try
                {
                    ...
                    responseMsg = ClinicSoft.Sync.IRDNepal.APIs.PostSalesBillToIRD(salesBill);
                }
                catch (Exception ex)
                {
                    responseMsg = "0";
        ...
        public static void SynchPhrmInvoiceToRemoteServer()
        {
            ...
            foreach (PHRMInvoiceTransactionModel invoice in unSyncedBillTxns)
            {
                ...
                try
                {
                    ...
                    responseMsg = ClinicSoft.Sync.IRDNepal.APIs.PostPhrmInvoiceToIRD(phrmInvoice);
                }
                catch (Exception ex)
                {
                    responseMsg = "0";
            ...
            try
            {
                ...
                switch (irdLogdata.BillType)
                {
                    ...
                    case "billing-sales-return":
                        {
                            string api_SalesReturnIRDNepal =  string.Empty;
        ...
        private static string GetPANNumber(IRDNepalDbContext dbContext, int? PatientId)
        {
            try
            {
                return dbContext.Patients.Where(s => s.PatientId == PatientId).FirstOrDefault().ClinicCardNo;
            }
            catch (Exception ex)
            {
                throw ex;
