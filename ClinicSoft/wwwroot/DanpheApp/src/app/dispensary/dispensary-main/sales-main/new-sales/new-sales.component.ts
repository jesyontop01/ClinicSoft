...
export class NewSalesComponent implements OnInit, OnDestroy {
  ...
  private LoadGlobalPatient(Patient: Patient) {
    if (Patient.PatientId > 0) {
      this.currentPatient.LPNo = Patient.LPNo;
  ...
  public SetAnonymous() {
    try {
      ...
      this.searchPatient = '';
      this.currSale.selectedPatient.LPNo = '';
  ...
  public SetSelectedPatientData(patInfo, afterRegistration: boolean, invoiceMode = 'sale') {
    try {
      ...
      if (patInfo.PatientId) {
        ...
        this.currSale.selectedPatient.LPNo = patInfo.LPNo;
  ...
  insurancePatientListFormatter(data: any): string {
    let html = `[${data['LPNo']}] | ${data["ShortName"]} | NSHI [ ${data['Ins_NshiNumber']}]`;
    return html;
  }
      ...
      this.pharmacyBLService.GetPatientByPatId($event.PatientId)
        .subscribe(res => {
          if (res.Status == "OK") {
            ...
            this.currSale.selectedPatient.LPNo = res.Results.LPNo;
