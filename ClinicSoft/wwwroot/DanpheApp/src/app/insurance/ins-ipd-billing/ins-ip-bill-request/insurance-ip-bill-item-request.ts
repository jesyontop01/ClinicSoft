...
@Component({
  ...
  templateUrl: './insurance-ip-bill-item-request.html'
})
export class InsuranceIpBillItemRequest implements OnInit {
  ...
  public AssignSelectedItem(index) {
    ...
    if (this.selectedItems[index]) {
      ...
      if (item) {
        ...
        this.billingTransaction.BillingTransactionItems[index].UpdateValidator("off", "RequestedBy", null);
      }
      else
  ...
  CheckItemProviderValidation(index: number) {
    ...
    // }
    if (this.selectedItems[index] && this.selectedItems[index].IsDoctorMandatory) {
      this.billingTransaction.BillingTransactionItems[index].UpdateValidator("on", "RequestedBy", "required");
    }
    else {
      this.billingTransaction.BillingTransactionItems[index].UpdateValidator("off", "RequestedBy", null);
    }
  ...
  patientListFormatter(data: any): string {
    let html = data["ShortName"] + ' [ ' + data['LPNo'] + ' ]';
    return html;
  }
  //start: list formatters
