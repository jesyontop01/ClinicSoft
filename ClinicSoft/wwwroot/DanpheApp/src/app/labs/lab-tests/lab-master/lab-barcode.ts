...
export class LabBarCodeComponent {
  ...
  patientListFormatter(data: any): string {
    let html = "";
    html = data["ShortName"] + ' [ ' + data['LPNo'] + ' ]' + ' - ' + data['Age'] + ' - ' + ' ' + data['Gender'];
    return html;
  }
  ...
  public GetAllDataByHospitalNumber($event) {
    if ($event.LPNo) {
      var patIdFromPatCode = this.allPatientList.find(p => p.LPNo == $event.LPNo);
  ...
  public ViewDetails(req): void {
    ...
    this.patientService.getGlobal().LPNo = req.LPNo;
