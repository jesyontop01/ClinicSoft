...
export class QrReaderComponent {
  ...
  LoadPatientInfo(patCode) {
    let patCodeFormatted = this.GetLPNoFromInput(patCode);
    ...
    if (patCodeFormatted) {
      if (this.department == 'billing') {
        ...
        this.onReadSuccess.emit({ LPNo: patCode });
      }
      this.setFocusToHospNoTextbox();
  ...
  GetLPNoFromInput(patCode: string): string {
    let retVal = "";
  ...
  handleKeyDown($event) {
    ...
    //if ($event.code == "Enter") {
    //let scannedText = this.LPNo;

    let patCode = this.GetLPNoFromInput(this.LPNo);
