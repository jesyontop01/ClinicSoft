...
namespace ClinicSoft.Controllers.Reporting
{
    ...
    public class ReportingController : Controller
    {
        ...
        public string PatientBillHistory(DateTime? FromDate, DateTime? ToDate, string LPNo)
        {
            ...
            try
            {
                ...
                PatientBillHistoryMaster paitbillhistory = reportingDbContext.PatientBillHistory(FromDate, ToDate, LPNo);
        ...
        public string DailyAppointmentReport(DateTime FromDate, DateTime ToDate, string Doctor_Name, string AppointmentType, string Diagnosis)
        {
        ...
        public string OutpatientMorbidityReport([FromRoute] DateTime FromDate, [FromRoute] DateTime ToDate)
        {
            ...
            try
            {
                ...
                var VisitList = (from pv in MrDbContext.PatientVisits
                                 where (pv.VisitDate.Date) >= (FromDate.Date)
                                 && (pv.VisitDate.Date) <= (ToDate.Date)
                ...
                var DiagnosisDuringVisit = (from d in MrDbContext.FinalDiagnosis
                                            ...
                                            where (pv.VisitDate.Date) >= (FromDate.Date) && (pv.VisitDate.Date) <= (ToDate.Date)
                                            select new
                                            {
                                                p.PatientId,
                                                pv.PatientVisitId,
