...
using ClinicSoft.ServerModel.AccountingModels;
using ClinicSoft.AccTransfer.Accounting;
            ...
            try
            {
                ...
                if (reqType == "voucher-report")
                {
                    ...
                    var OpeningBalanceData = (from t in accountingDBContext.Transactions
                                              ...
                                              t.TransactionDate.Value.Date == FromDate && t.FiscalyearId == fisc.FiscalYearId
                                              ...
                                              select new
                                              {
                                                  Amountdr = x.Where(b => b.ti.DrCr == true && b.t.TransactionDate.Value.Date >= b.fisc.StartDate && b.t.TransactionDate.Value.Date < FromDate).Select(a => (int?)a.ti.Amount).DefaultIfEmpty(0).Sum(),
                                                  Amountcr = x.Where(b => b.ti.DrCr == false && b.t.TransactionDate.Value.Date >= b.fisc.StartDate && b.t.TransactionDate.Value.Date < FromDate).Select(a => (int?)a.ti.Amount).DefaultIfEmpty(0).Sum(),
                                                  x.Key.FiscalYearId,
                                              }).ToList();
                    ...
                    var txnList = (from txn in accountingDBContext.Transactions
                                   ...
                                   txn.IsActive == true && txn.TransactionDate.Value.Date >= FromDate && txn.TransactionDate.Value.Date <= ToDate
                                   ...
                                   select new
                                   {
                                       ...
                                       TransactionDate = txn.TransactionDate.Value.Date,// txn.TransactionDate.ToString("dd/mm/yyyy"),                                       
                ...
                else if (reqType == "ledger-report")
                {
                    ...
                    var LedgerData = (from t in accountingDBContext.Transactions
                                      ...
                                      (ti.LedgerId == ledgerId) && t.TransactionDate.Value.Date >= FromDate && t.TransactionDate.Value.Date <= ToDate
                                      select new
                                      {
                                          t.TransactionId,
                                          t.TransactionDate,
                    ...
                    var dataList1 = (from t in accountingDBContext.Transactions
                                     ...
                                     ti.HospitalId == currentHospitalId
                                     && l.HospitalId == currentHospitalId &&
                                     (ti.LedgerId == ledgerId) && (t.TransactionDate.Value.Date) >= (openingBalanceFiscalYear.StartDate.Date) && (t.TransactionDate.Value.Date) <= ToDate
                                    ...
                                    into x
                                     select new
                                     {
                                         ...
                                         AmountDr = x.Where(b => b.ti.DrCr == true && b.t.TransactionDate.Value.Date < FromDate).Select(a => (double?)a.ti.Amount).DefaultIfEmpty(0).Sum() + ((LedgerData.Count == 0 && led.LedgerId > 0 && led.DrCr == true) ? led.OpeningBalance : 0),
                                         AmountCr = x.Where(b => b.ti.DrCr == false && b.t.TransactionDate.Value.Date < FromDate).Select(a => (double?)a.ti.Amount).DefaultIfEmpty(0).Sum() + ((LedgerData.Count == 0 && led.LedgerId > 0 && led.DrCr == false) ? led.OpeningBalance : 0),
                                     }).ToList();
                                  ...
                                  select new
                                  {
                                      ...
                                      TransactionItems = (showTxnItemLevel == true) ? null :
                                                                                        (from txnItm in accountingDBContext.TransactionItems
                                                                                         ...
                                                                                         && txn.TransactionDate.Value.Date >= FromDate && txn.TransactionDate.Value.Date <= ToDate
                                                                        ...
                                                                        select new
                                                                        {
                                                                            ...
                                                                            LedgersList = (from ti in accountingDBContext.TransactionItems
                                                                                           ...
                                                                                           select new
                                                                                           {
                                                                                               ...
                                                                                               x4.Key.Code,
                                                                                               OpenBal = x4.Where(b => b.lgroup.LedgerGroupName == "Cash In Hand" && (b.t.TransactionDate.Value.Date) < FromDate).Select(a => (int?)a.ti.Amount).DefaultIfEmpty(0).Sum(),
                                                                                               Amountdr = x4.Where(b => b.ti.DrCr == true && (b.t.TransactionDate.Value.Date) >= FromDate && (b.t.TransactionDate.Value.Date) <= ToDate).Select(a => (int?)a.ti.Amount).DefaultIfEmpty(0).Sum(),
                                                                                               Amountcr = x4.Where(b => b.ti.DrCr == false && (b.t.TransactionDate.Value.Date) >= FromDate && (b.t.TransactionDate.Value.Date) <= ToDate).Select(a => (int?)a.ti.Amount).DefaultIfEmpty(0).Sum(),
                                                                                           }).ToList()
                                                                        }).ToList()
                                                 }).ToList()
                                  }).ToList();
                ...
                else if (reqType == "bank-reconcilation-report")
                {
                    ...
                    var dataList1 = (from t in accountingDBContext.Transactions
                                    ...
                                    into x
                                     ...
                                     {
                                         FiscalYearId = fYearId,
                                         AmountDr = x.Where(b => b.ti.DrCr == true && (b.t.TransactionDate.Value.Date) < FromDate).Select(a => (double?)a.ti.Amount).DefaultIfEmpty(0).Sum() + ((transactionRecords.Count == 0 && led.LedgerId > 0 && led.DrCr == true) ? led.OpeningBalance : 0),
                                         AmountCr = x.Where(b => b.ti.DrCr == false && (b.t.TransactionDate.Value.Date) < FromDate).Select(a => (double?)a.ti.Amount).DefaultIfEmpty(0).Sum() + ((transactionRecords.Count == 0 && led.LedgerId > 0 && led.DrCr == false) ? led.OpeningBalance : 0),
                                     }).ToList();
