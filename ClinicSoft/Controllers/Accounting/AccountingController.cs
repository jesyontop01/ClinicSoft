        ...
        public string Get(int? voucherId, int transactionId, int ledgerId, string voucherNumber, string ledgerName, DateTime FromDate, DateTime ToDate, int? sectionId, string ledgerType, int referenceId, DateTime SelectedDate, int FiscalYearId, DateTime transactiondate)
        {
            ...
            try
            {
                ...
                else if (reqType == "ledger-list")
                {
                    ...
                    //var connection = accountingDBContext.Database.GetDbConnection() as SqlConnection;

                    //if(connection != null)
                    //{
                    //    connection.Open();

                    //    using (var command = new SqlCommand("SP_ACC_GetLedgerList", connection))
                    //    {
                    ...
                    //        {
                    //            spDataTable = new DataTable();
                    //            spDataTable.Load(reader);
                    //        }
                    //    }
                    //}
                ...
                else if (reqType == "inventory-to-accounting")
                {
                    ...
                    List<TransactionModel> result = AccountingTransferData.GetMapAndTransferDataSectionWise(1, SelectedDate, FiscalYearId, currentHospitalId);
                    var unavailableLedger = AccountingTransferData.GetUnavailableLedgerList();
                   ...
                   AccountingTransferData.ClearUnavailableLedgerList();
                }
                #endregion
                ...
                else if (reqType == "billing-to-accounting")
                {
                    RbacUser currentUser = HttpContext.Session.Get<RbacUser>("currentuser");
                    AccountingTransferData obj = new AccountingTransferData(connString, currentUser.EmployeeId, currentHospitalId);
                   List<TransactionModel> res = AccountingTransferData.GetMapAndTransferDataSectionWise(2, SelectedDate, FiscalYearId, currentHospitalId);
                ...
                else if (reqType == "pharmacy-to-accounting")
                {
                    RbacUser currentUser = HttpContext.Session.Get<RbacUser>("currentuser");
                    AccountingTransferData obj = new AccountingTransferData(connString, currentUser.EmployeeId, currentHospitalId);
                    List<TransactionModel> result = AccountingTransferData.GetMapAndTransferDataSectionWise(3, SelectedDate, FiscalYearId, currentHospitalId);
                ...
                else if (reqType == "incentive-to-accounting")
                {
                    RbacUser currentUser = HttpContext.Session.Get<RbacUser>("currentuser");
                    AccountingTransferData obj = new AccountingTransferData(connString, currentUser.EmployeeId, currentHospitalId);
                    List<TransactionModel> result = AccountingTransferData.GetMapAndTransferDataSectionWise(5, SelectedDate, FiscalYearId, currentHospitalId);
                    var unavailableLedger = AccountingTransferData.GetUnavailableLedgerList();
                    if (unavailableLedger.Count > 0)
                    {
                        responseData.Status = "Failed";
                  ...
                  if(connection != null)
                    {
                        ...
                        using (var command = new SqlCommand("SP_ACC_GetAllEmployee_LedgerList", connection))
                        {
                            command.CommandType = CommandType.StoredProcedure;
                            command.Parameters.AddRange(paramList.ToArray());
                    ...
                    //using (var connection = new SqlConnection(accountingDBContext.Database.Connection.ConnectionString))
                    //{
                    //    connection.Open();
            ...
            try
            {
                ...
                else if (reqType == "post-account-closure")
                {
                    ...
                    if (nextFiscalYearId > 0)
                    {
                        ...
                        if(connection != null)
                        {
                            ...
                            using (var command = new SqlCommand("SP_INCTV_PaymentInfo_Update", connection))
                            {
                                command.CommandType = CommandType.StoredProcedure;
                                command.Parameters.AddRange(paramList.ToArray());
                ...
                else if (reqType == "post-reverse-transaction")
                {
                    try
                    {
                        ...
                        List<TransactionModel> txnList = accountingDBContext.Transactions.Where(a => a.TransactionDate.Value.Date == reverseTransactionData.TransactionDate && a.SectionId == reverseTransactionData.Section
