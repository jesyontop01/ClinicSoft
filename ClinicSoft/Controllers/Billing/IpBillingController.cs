                ...
                if (reqType == "list-ip-patients")
                {
                    var ipPatients = (from adm in dbContext.Admissions.Include(a => a.Visit.Patient)
                                      ...
                                      select new
                                      {
                                          ...
                                          PatientNo = patient.ClinicCardNo,
                                          patient.LPNo,
                                          patient.Gender,
                    ...
                    if (isAutoAddBedItems)
                    {
                        ...
                        if ((allBedItemsOfPatientByVisit != null) && allBedItemsOfPatientByVisit.Any())
                        {
                            ...
                            var allBedInfoOfPatient = billingDbContext.PatientBedInfos.AsNoTracking().Where(b => b.IsActive && (b.PatientVisitId == patientVisitId))
                                                        .Select(d => new
                                                        {
                                                            StartedOn = (d.StartedOn.Value.Date),
                            ...
                            if ((allBedInfoOfPatient != null) && (allBedInfoOfPatient.Count > 0))
                            {
                                ...
                                if (bedDataGroupedByStartDate.Count > 1)
                                {
                                    for (int i = 1; i < bedDataGroupedByStartDate.Count; i++)
                                    {
                                        ...
                                        {
                                            bedWithDaysCount[finalBedOfPreviousDay.BedFeatureId] += (finalBedOfCurrentDay.StartedOn.Date - finalBedOfPreviousDay.StartedOn.Date).Days;
                                            if ((i + 1) == bedDataGroupedByStartDate.Count)
                                            {
                                                bedWithDaysCount[finalBedOfPreviousDay.BedFeatureId] += (System.DateTime.Now.Date - finalBedOfCurrentDay.StartedOn.Date).Days;
                                            }
                                        }
                                        else //if previous day and current day both have different BedFeature
                                        {
                                            diffInDays = (System.DateTime.Now - finalBedOfCurrentDay.StartedOn.Date).Days;
                                            bedWithDaysCount[finalBedOfPreviousDay.BedFeatureId] += (finalBedOfCurrentDay.StartedOn.Date - finalBedOfPreviousDay.StartedOn.Date).Days;

                                            //if new bed started today
                                            if (finalBedOfCurrentDay.StartedOn.Date == System.DateTime.Now.Date)
                                            {
                                            ...
                                            else
                                            {
                                                ...
                                                if ((i + 1) == bedDataGroupedByStartDate.Count)
                                                {
                                                    bedWithDaysCount[finalBedOfCurrentDay.BedFeatureId] += (System.DateTime.Now.Date - finalBedOfCurrentDay.StartedOn.Date).Days;
                                                }
                                            }
                                        }
                                    }
                                ...
                                {
                                    if (finalBedOfPreviousDay.StartedOn.Date == System.DateTime.Now.Date)
                                    {
                                        bedWithDaysCount[finalBedOfPreviousDay.BedFeatureId] = 1;
                                    }
                                    else
                                    {
                                        var daysDiff = (System.DateTime.Now - finalBedOfPreviousDay.StartedOn.Date).Days;
