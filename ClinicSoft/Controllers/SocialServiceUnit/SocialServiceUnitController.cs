    ...
    public class SocialServiceUnitController : CommonController
    {
        ...
        public async Task<IActionResult> GetAllSsuPatients(string search = "")
        {
            ...
            try
            {
                ...
                var allPats = (from pat in ssuDbContext.Patients.Include("Visits")
                               ...
                               where pat.IsStaff == true && ((pat.FirstName + " " + (string.IsNullOrEmpty(pat.MiddleName) ? "" : pat.MiddleName + " ") + pat.LastName + " " + pat.Address + " " + pat.PhoneNumber + " " + pat.Address + pat.FirstName + " " + pat.Address + pat.LPNo).Contains(search))
                               select new
                               {
        ...
        public IActionResult PostSsuPatientInformation()
        {
            ...
            try
            {
                ...
                patDetails.LPNo = newPatientNumber.LPNo;
                patDetails.LPNo = newPatientNumber.LPNo;
                ...
                using (var dbContextTransaction = dbContext.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        if(patDetails.IsStaff == true && patDetails.SSU_Information != null )
                        {
            ...
            try
            {
                ...
                using (var dbContextTransaction = dbContext.Database.BeginTransaction())
                {
                    try                    
                    {
                        ...
                        dbContext.Entry(patDetails).Property(x => x.IsStaff).IsModified = true;
                        ...
                        if (patDetails.IsStaff == true && patDetails.SSU_Information != null && patDetails.SSU_Information.SSU_InfoId<=0)
                        {
                            SSU_InformationModel newSsuPat = new SSU_InformationModel();
                            newSsuPat = patDetails.SSU_Information;
                            newSsuPat.PatientId = patDetails.PatientId;
                        ...
                        else if (patDetails.IsStaff == true && patDetails.SSU_Information != null && patDetails.SSU_Information.SSU_InfoId > 0)
                        {
