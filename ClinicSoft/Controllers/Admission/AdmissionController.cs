...
using ClinicSoft.ViewModel.ADT;
            ...
            try
            {
                ...
                else if (reqType == "DischargedPatientsList")
                {
                    ...
                    var result = (from admission in dbContext.Admissions.Include(a => a.Visit.Patient)
                                  ...
                                  where admission.AdmissionStatus == admissionStatus && (admission.DischargeDate.Value.Date) >= FromDate && (admission.DischargeDate.Value.Date) <= ToDate
                                  ...
                                  select new
                                  {
                                      ...
                                      LPNo = admission.Visit.Patient.LPNo,
                ...
                else if (reqType == "getAdmittedList")
                {
                    ...
                    var result = (from admission in dbContext.Admissions.Include(a => a.Visit.Patient)
                                  ...
                                  + admission.Visit.Patient.LastName + admission.Visit.Patient.LPNo + admission.Visit.Patient.PhoneNumber).Contains(search)
            ...
            try
            {
                ...
                if (reqType == "Admission")
                {
                    ...
                    if (clientAdt.AdmittingDoctorId.HasValue && clientAdt.AdmittingDoctorId > 0)
                    {
                        ...
                        var patcode = dbContext.Patients.ToList().Where(a => a.PatientId == clientAdt.PatientId).Select(a => a.LPNo).FirstOrDefault();
                        var smsmsg = "Dear,";
                    ...
                    if (clientBedInfo != null)
                    {
                        ...
                        if (serverBedInfo != null)
                        {
                            ...
                            PatientBedInfo previousbedinfo = (from patbed in dbContext.PatientBedInfos
                                                              where patbed.PatientVisitId == clientBedInfo.PatientVisitId && (patbed.EndedOn.Value.Date) == (serverBedInfo.StartedOn.Value.Date)
                                                              select patbed).FirstOrDefault();

                            PatientBedInfo nextbedinfo = (from patbedinfo in dbContext.PatientBedInfos
                                                          where patbedinfo.PatientVisitId == clientBedInfo.PatientVisitId && (patbedinfo.StartedOn.Value.Date) == (serverBedInfo.EndedOn.Value.Date)
