...
namespace ClinicSoft.Controllers
{
    public class PatientController : CommonController
    {
        ...
        //private string GetLPNo(int patientNo)
        //{
            ...
            try
            {
                ...
                else if (reqType == "patient-search-by-text") // Vikas: 17th June 2019 :added real time search.
                {
                    ...
                    var allPats = (from pat in patDbContext.Patients.Include("Visits")
                                   ...
                                   && ((pat.FirstName + " " + (string.IsNullOrEmpty(pat.MiddleName) ? "" : pat.MiddleName + " ") + pat.LastName + " " + pat.Address + " " + pat.PhoneNumber + " " + pat.Address + pat.FirstName + " " + pat.Address + pat.LPNo).Contains(search))
                                   select new
                                   {
                                       PatientId = pat.PatientId,
                                       LPNo = pat.LPNo,
        ...
        public string Post()
        {
            ...
            try
            {
                ...
                if (reqType == "patient")
                {
                    ...
                    responseData.Results = new PatientModel() { LPNo = clientPatModel.LPNo, PatientId = clientPatModel.PatientId };
                    responseData.Status = "OK";
                }
                ...
                else if (reqType == "gov-insurance-patient")
                {
                    ...
                    PatientModel govInsNewPatient = ClinicSoft.Controllers.PatientBL.GetPatientModelFromPatientVM(govInsClientPatObj, connString, patDbContext);
                    InsuranceModel patInsInfo = ClinicSoft.Controllers.PatientBL.GetInsuranceModelFromInsPatientVM(govInsClientPatObj, currentUser.EmployeeId);
                ...
                else if (reqType == "billing-out-patient")
                {
                    ...
                    PatientModel newPatient = ClinicSoft.Controllers.PatientBL.GetPatientModelFromPatientVM(billingOutPatVM, connString, patDbContext);
        ...
        private void GeneratePatientNoAndSavePatient(PatientDbContext patDbContext, PatientModel clientPatModel, string connString)
        {
            try
            {
                ...
                clientPatModel.ClinicCardNo = newPatientNumber.ClinicCardNo;
                clientPatModel.LPNo = newPatientNumber.LPNo;
                patDbContext.SaveChanges();
            }
            catch (Exception ex)
        ...
        public string Put(string reqType)
        {
            ...
            try
            {
                ...
                //    patDbContext.Entry(objFromClient).Property(u => u.LPNo).IsModified = false;
                //    patDbContext.Entry(objFromClient).Property(u => u.ClinicCardNo).IsModified = false;//sud: 15Aug'18
