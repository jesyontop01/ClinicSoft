            ...
            string search)
        {
            ...
            try
            {
                ...
                else if (reqType == "CheckIfApptExistForDepartmentOnDate")
                {
                    ...
                    var visitItem = dbContext.Visits.Where(v => (v.VisitDate.Date == (requestDate.Date)) && (v.DepartmentId == departmentId) && (v.PatientId == patientId && v.BillingStatus.ToLower() != ENUM_BillingStatus.returned && v.BillingStatus.ToLower() != ENUM_BillingStatus.cancel)).AsEnumerable();
                ...
                else if (reqType == "patient-visitHistorylist")
                {
                    ...
                    var visitList = (from visit in dbContext.Visits
                                     where visit.PatientId == patientId
                                   //DbFunctions.TruncateTime(defaultLastDateToShow)
                                   && (visit.VisitDate.Date == (DateTime.Now.Date) || followup == true)
                                     && visit.BillingStatus != ENUM_BillingStatus.returned // "returned"
                                     select visit).ToList();
                    responseData.Results = visitList;
                }
            ...
            try
            {
                if (clientTransaction.BillingTransactionItems != null && clientTransaction.BillingTransactionItems.Count > 0)
                {
                    ...
                    if (realTimeRemoteSyncEnabled)
                    {
                        ...
                        ClinicSoft.Controllers.VisitBL.SyncBillToRemoteServer(clientTransaction, "sales", visitDbContext);
                    }
                }
                else
                {
