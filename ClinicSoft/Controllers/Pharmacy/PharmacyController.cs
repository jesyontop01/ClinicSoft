...
using System.IO;
using ClinicSoft.ServerModel.CommonModels;
            ...
            int settlementId, int invoiceid, int? gdprintId, int invoiceretid, string CancelRemarks, int storeId, string invoiceNo)
        {
            ...
            try
            {
                ...
                else if (reqType == "allItemsStockDetails")
                {
                    ...
                    //                 };
                    var totalStock = phrmdbcontext.StoreStocks
                  .Join(phrmdbcontext.PHRMItemMaster, stk => stk.ItemId, itm => itm.ItemId, (stk, itm) => new { stk, itm })
                  .GroupJoin(phrmdbcontext.PHRMGenericModel,
                             x => x.itm.GenericId,
                             gen => gen.GenericId,
                             (x, gens) => new { x.stk, x.itm, genLj = gens.DefaultIfEmpty() })
                  .Where(x => x.stk.StoreId == DispensaryId)
                  .GroupBy(x => new { x.stk.ItemId, x.stk.StockMaster.BatchNo, x.stk.StockMaster.ExpiryDate, x.stk.StockMaster.CostPrice, x.stk.StockMaster.MRP, x.itm.ItemName })
                  .Select(stkGrouped => new GetDispensaryStockVm
                  {
                      ItemId = stkGrouped.Key.ItemId,
                      GenericName = (stkGrouped.FirstOrDefault().genLj.FirstOrDefault() != null) ? stkGrouped.FirstOrDefault().genLj.FirstOrDefault().GenericName : "N/A",
                      ItemName = stkGrouped.Key.ItemName,
                      BatchNo = stkGrouped.Key.BatchNo,
                      ExpiryDate = stkGrouped.Key.ExpiryDate.Value,
                      MRP = stkGrouped.Key.MRP,
                ...
                else if (reqType != null && reqType.ToLower() == "listpatientunpaidtotal")
                {
                    ...
                    var allPatientCreditReceipts = (from bill in phrmdbcontext.PHRMInvoiceTransactionItems.Where(i => i.StoreId == DispensaryId)
                                                    ...
                                                    group bill by new { pat.PatientId, pat.LPNo, pat.FirstName, pat.LastName, pat.MiddleName, pat.DateOfBirth, pat.Gender, bill.InvoiceId, pat.PhoneNumber, bill.CreatedBy, billingUser.UserName, pat.Address, subdiv.CountrySubDivisionName, pat.ClinicCardNo } into p
                                                    select new
                                                    {
 ...
 */
        ...
        public string Post()
        {
            ...
            try
            {
                ...
                else if (reqType == "outdoorPatRegistration")
                {
                    ...
                    phrmdbcontext.SaveChanges();
                    patientData.LPNo = this.GetLPNo(patientData.PatientId);
                    patientData.ClinicCardNo = this.GetPatientNo(patientData.PatientId);
                    phrmdbcontext.SaveChanges();
        ...
        public string GetPatientNo(int patientId)
        {
            try
            {
                if (patientId != 0)
        ...
        public string GetLPNo(int patientId)
        {
            try
            {
                if (patientId != 0)
                {
                    ...
                    PatientDbContext patientDbContext = new PatientDbContext(connString);
                    var maxPatNo = patientDbContext.Patients.DefaultIfEmpty().Max(p => p == null ? "" : p.LPNo);
                    newPatNo = maxPatNo;
