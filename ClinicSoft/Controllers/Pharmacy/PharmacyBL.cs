...
using ParameterModel = ClinicSoft.Core.Parameters.ParameterModel;
...
namespace ClinicSoft.Controllers
{
    public class PharmacyBL
    {
        ...
        public static DataTable SearchPatient(string SearchText, bool IsInsurance, PharmacyDbContext phrmDBContext)
        {
            ...
            //               && ((pat.ShortName + pat.PhoneNumber ?? "" + " " + pat.LPNo + pat.Ins_NshiNumber ?? "").Contains(SearchText))
        ...
        public static int RegisterPatient(PHRMPatient patient, PharmacyDbContext db, PatientDbContext patientDb, CoreDbContext coreDb)
        {
            try
            {
                ...
                patient.ClinicCardNo = GetLatestPatientNo(patientDb);
                patient.LPNo = GetLPNo(patient.ClinicCardNo, patientDb, coreDb);
        ...
        public static string GetLatestPatientNo(PatientDbContext patientDbContext)
        {
            try
            {
        ...
        public static string GetLPNo(string patientNo, PatientDbContext patientDbContext, CoreDbContext coreDbContext)
        {
            try
            {
                NewPatientUniqueNumbersVM retValue = new NewPatientUniqueNumbersVM();
                string newPatNo = patientNo;
                string newPatCode = "";
        ...
        public static int? GetDepositReceiptNo(string connString)
        {
            using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions { IsolationLevel = System.Transactions.IsolationLevel.ReadUncommitted }))
            {
                ...
                ClinicSoft.DalLayer.PharmacyDbContext phrmDbContext = new DalLayer.PharmacyDbContext(connString);
        ...
        public static void SyncPHRMBillInvoiceToRemoteServer(object billToPost, string billType, PharmacyDbContext dbContext)
        {
            ...
            if (billType == "phrm-invoice")
            {
                ...
                try
                {
                    ...
                    responseMsg = ClinicSoft.Sync.IRDNepal.APIs.PostPhrmInvoiceToIRD(invoicebill);
                }
                catch (Exception ex)
                {
                    responseMsg = "0";
            ...
            else if (billType == "phrm-invoice-return")
            {
                ...
                try
                {
                    ...
                    responseMsg = ClinicSoft.Sync.IRDNepal.APIs.PostPhrmInvoiceReturnToIRD(salesRetBill);
                }
                catch (Exception ex)
                {
                    responseMsg = "0";
        ...
        private static string GetPANNumber(PharmacyDbContext dbContext, int? PatientId)
        {
            try
            {
                return dbContext.PHRMPatient.Where(s => s.PatientId == PatientId).FirstOrDefault().ClinicCardNo;
            }
            catch (Exception ex)
            {
                throw ex;
