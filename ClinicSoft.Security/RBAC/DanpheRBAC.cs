...
namespace ClinicSoft.Security
{
    ...
    public class RBAC
    {
        ...
        //This get all routes from db
        public static List<DanpheRoute> GetAllNewUserRoutes(int landingRouteId)
        {
            List<DanpheRoute> retList = (List<DanpheRoute>)DanpheCache.Get("RBAC-Routes-All");
            if (retList == null)
            {
                RbacDbContext dbContext = new RbacDbContext(connStringName);
                retList = dbContext.Routes.Where(x=>x.RouteId.Equals(landingRouteId)).ToList();
                DanpheCache.Add("RBAC-Routes-All", retList, cacheExpiryMinutes);
            }
            return retList;
        ...
        //don't get hidden routes if it's set to false.
        public static List<DanpheRoute> GetRoutesForNewUser(int userId, bool getHiearrchy)
        {
            List<DanpheRoute> allRoutes = new List<DanpheRoute>();
        ...
        public static UserRoleMap GetUserRoleMap(int userid)
        {
            var result = new UserRoleMap();
            if(userid > 0)
            {
                RbacDbContext rbacDbcontxt = new RbacDbContext(connStringName);

                var resultMap = rbacDbcontxt.UserRoleMaps.Where(u=>u.UserId == userid).FirstOrDefault();
                if(resultMap != null)
                {
                ...
                {
                    var results = new UserRoleMap();

                    rbacDbcontxt.UserRoleMaps.Add(result);
        ...
        public static RbacUser CreateUser(RbacUser user)
        {
            RbacDbContext rbacDbcontxt = new RbacDbContext(connStringName);
            ...
            if (user != null)
            {
                ...
                rbacDbcontxt.Users.Add(result);
                rbacDbcontxt.SaveChanges();

                var defaultRole = rbacDbcontxt.Roles.Where(r=>r.RoleName.Equals("User")).FirstOrDefault(); 
               
                //if (rbacDbcontxt.SaveChanges() > 0)
                //{
                    result = rbacDbcontxt.Users.Where(u => u.Email == user.Email).FirstOrDefault();
                    if (defaultRole != null)
                    {
                ...
                //}
                roleMap.CreatedBy = user.CreatedBy;
                roleMap.CreatedOn = DateTime.Now;
                    //results.User

                    rbacDbcontxt.UserRoleMaps.Add(roleMap);
                    rbacDbcontxt.SaveChanges();
